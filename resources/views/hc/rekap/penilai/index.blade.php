              ...
              <div class="card-header">
                <div class="d-flex bd-highlight flex-row">
                  <div class="bd-highlight px-2">
                    <button id="btnRekapPenilaiModal"
                            type="button"
                            class="btn btn-info">
              ...
              <div class="card-body">
                <table id="dataTablesRekap2"
                  ...
                  <thead>
                    <tr>
                      <th colspan='2'></th>
                      <th colspan='2'>Identitas Dinilai</th>
                  ...
                  <tbody>
                    @foreach ($data_dinilai as $p)
                      <tr>
                        ...
                        @if (Str::remove(' ', $p->identitas_dinilai->level_jabatan) == 'Direksi' ||
                                Str::remove(' ', $p->identitas_dinilai->level_jabatan) == 'IA' ||
                                Str::remove(' ', $p->identitas_dinilai->level_jabatan) == 'IB' ||
                                Str::remove(' ', $p->identitas_dinilai->level_jabatan) == 'IC' ||
                                Str::remove(' ', $p->identitas_dinilai->level_jabatan) == 'IANS')
                          <td>(<b>40%</b>) {{ round($p->sum_k1 * 100, 2) }}</td>
                        @elseif(Str::remove(' ', $p->identitas_dinilai->level_jabatan) == 'II' ||
                                Str::remove(' ', $p->identitas_dinilai->level_jabatan) == 'IINS')
                          <td>(<b>35%</b>) {{ round($p->sum_k1 * 100, 2) }}</td>
                        @elseif(Str::remove(' ', $p->identitas_dinilai->level_jabatan) == 'III' ||
                                Str::remove(' ', $p->identitas_dinilai->level_jabatan) == 'IIINS' ||
                                Str::remove(' ', $p->identitas_dinilai->level_jabatan) == 'IIII')
                          <td>(<b>30%</b>) {{ round($p->sum_k1 * 100, 2) }}</td>
                        @elseif(Str::remove(' ', $p->identitas_dinilai->level_jabatan) == 'IV' ||
                                Str::remove(' ', $p->identitas_dinilai->level_jabatan) == 'IVA(III)' ||
                                Str::remove(' ', $p->identitas_dinilai->level_jabatan) == 'IVA(IIINS)' ||
                                Str::remove(' ', $p->identitas_dinilai->level_jabatan) == 'IVA')
                          <td>(<b>10%</b>) {{ round($p->sum_k1 * 100, 2) }}</td>
                        @elseif(Str::remove(' ', $p->identitas_dinilai->level_jabatan) == 'IVB' ||
                                Str::remove(' ', $p->identitas_dinilai->level_jabatan) == 'V' ||
                                Str::remove(' ', $p->identitas_dinilai->level_jabatan) == 'THL')
                          <td>(<b>0%</b>) {{ round($p->sum_k1 * 100, 2) }}</td>
                        @else
                          <td>Tidak Masuk</td>
                        @endif
                        @if (Str::remove(' ', $p->identitas_dinilai->level_jabatan) == 'Direksi' ||
                                ...
                                Str::remove(' ', $p->identitas_dinilai->level_jabatan) == 'IC' ||
                                Str::remove(' ', $p->identitas_dinilai->level_jabatan) == 'IANS')
                          <td>(<b>35%</b>) {{ round($p->sum_k2 * 100, 2) }}</td>
                        @elseif(Str::remove(' ', $p->identitas_dinilai->level_jabatan) == 'II' ||
                                Str::remove(' ', $p->identitas_dinilai->level_jabatan) == 'IINS')
                          <td>(<b>40%</b>) {{ round($p->sum_k2 * 100, 2) }}</td>
                        @elseif(Str::remove(' ', $p->identitas_dinilai->level_jabatan) == 'III' ||
                                Str::remove(' ', $p->identitas_dinilai->level_jabatan) == 'IIINS' ||
                                Str::remove(' ', $p->identitas_dinilai->level_jabatan) == 'IIII')
                          <td>(<b>45%</b>) {{ round($p->sum_k2 * 100, 2) }}</td>
                        @elseif(Str::remove(' ', $p->identitas_dinilai->level_jabatan) == 'IV' ||
                                Str::remove(' ', $p->identitas_dinilai->level_jabatan) == 'IVA(III)' ||
                                Str::remove(' ', $p->identitas_dinilai->level_jabatan) == 'IVA(IIINS)' ||
                                Str::remove(' ', $p->identitas_dinilai->level_jabatan) == 'IVA')
                          <td>(<b>60%</b>) {{ round($p->sum_k2 * 100, 2) }}</td>
                        @elseif(Str::remove(' ', $p->identitas_dinilai->level_jabatan) == 'IVB' ||
                                Str::remove(' ', $p->identitas_dinilai->level_jabatan) == 'V' ||
                                Str::remove(' ', $p->identitas_dinilai->level_jabatan) == 'THL')
                          <td>(<b>65%</b>) {{ round($p->sum_k2 * 100, 2) }}</td>
                        @else
                          <td>Tidak Masuk</td>
                        @endif
                        @if (Str::remove(' ', $p->identitas_dinilai->level_jabatan) == 'Direksi' ||
                                ...
                                Str::remove(' ', $p->identitas_dinilai->level_jabatan) == 'IC' ||
                                Str::remove(' ', $p->identitas_dinilai->level_jabatan) == 'IANS')
                          <td>(<b>25%</b>) {{ round($p->sum_k3 * 100, 2) }}</td>
                        @elseif(Str::remove(' ', $p->identitas_dinilai->level_jabatan) == 'II' ||
                                Str::remove(' ', $p->identitas_dinilai->level_jabatan) == 'IINS')
                          <td>(<b>25%</b>) {{ round($p->sum_k3 * 100, 2) }}</td>
                        @elseif(Str::remove(' ', $p->identitas_dinilai->level_jabatan) == 'III' ||
                        ...
                        @elseif(Str::remove(' ', $p->identitas_dinilai->level_jabatan) == 'IV' ||
                                ...
                                Str::remove(' ', $p->identitas_dinilai->level_jabatan) == 'IVA(IIINS)' ||
                                Str::remove(' ', $p->identitas_dinilai->level_jabatan) == 'IVA')
                          <td>(<b>30%</b>) {{ round($p->sum_k3 * 100, 2) }}</td>
                        @elseif(Str::remove(' ', $p->identitas_dinilai->level_jabatan) == 'IVB' ||
                                Str::remove(' ', $p->identitas_dinilai->level_jabatan) == 'V' ||
                                Str::remove(' ', $p->identitas_dinilai->level_jabatan) == 'THL')
                          <td>(<b>35%</b>) {{ round($p->sum_k3 * 100, 2) }}</td>
                        @else
                          <td>Tidak Masuk</td>
                        @endif
                        <td>
                          <a href="{{ route('dp3-total-detail', ['dinilai' => $p->npp_dinilai]) }}"
                             class="btn btn-sm btn-outline-secondary">
                            <i class="fas fa-eye px-1"></i>Detil
                          </a>
                        </td>
                        <td>
...
@pushOnce('modals')
    ...
    <div class="modal-dialog modal-lg modal-dialog-centered modal-dialog-scrollable">
      <div class="modal-content">
        <div class="modal-header">
          <h5 id="staffModalLabel"
              class="modal-title">List Staff</h5>
        </div>
  ...
  <div id="rekananModal"
       class="modal fade"
       tabindex="-1"
       aria-labelledby="rekananModalLabel"
       aria-hidden="true">
    <div class="modal-dialog modal-lg modal-dialog-centered modal-dialog-scrollable">
      <div class="modal-content">
        <div class="modal-header">
          <h5 id="rekananModalLabel"
              class="modal-title">List Rekanan</h5>
        </div>
        <div class="modal-body">
  ...
  <script>
    $(document).ready(function(e) {
      ...
      $('#btnRekapPenilaiModal').on('click', function(ev) {
        ...
        alertswal(
          'anda yakin', 'anda melakukan rekap semua nilai dp3', 'warning', 'Iya'
        );
      });
    });
  </script>
