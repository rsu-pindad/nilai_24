...
class SyncRekanan implements ShouldQueue
{
    use Batchable, Dispatchable, InteractsWithQueue, Queueable, SerializesModels;

    protected $skorDataJawaban;
    ...
    {
        $this->skorDataJawaban = ScoreJawaban::get()->groupBy('aspek_id')->toArray();
    ...
    public function handle(): void
    {
        try {
                ...
                foreach ($chunk as $key => $value) {
                    if ($chunk[$key]['npp_penilai'] != $chunk[$key]['npp_dinilai']) {
                        ...
                        if ($idRelasiKaryawan) {
                            ...
                            $findrelasi = $this->findRelasi('rekanan', $idRelasiKaryawan['id']);
                            if ($findrelasi != '') {
                                $findrelasi->toArray();
                                if ($findrelasi['npp_selevel'] != $chunk[$key]['npp_dinilai']) {
                                    continue;
                                } elseif ($findrelasi['npp_selevel'] == $chunk[$key]['npp_dinilai']) {
            ...
            // Pembagian Segment
            $kepemimpinan = $this->skorDataJawaban[1];
            $perilaku     = $this->skorDataJawaban[2];
            $sasaran      = $this->skorDataJawaban[3];
            $tempData     = [];
